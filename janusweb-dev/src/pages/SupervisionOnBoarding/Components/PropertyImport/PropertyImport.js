...
const PropertyImport = () => {
  ...
  const { setProperties } = useOnboarding();
  ...
  const checkDataValidation = (data) => {
    ...
    data.forEach((property) => {
      ...
      console.log({ property_code, legal_name, name });
      // Basic validation
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (!data || data.length < 2) {
          // Check if data exists and has header + at least one row
          toast.error(
            t(
              "common.pages.The file could not be read, please check that it follows the template"
            )
